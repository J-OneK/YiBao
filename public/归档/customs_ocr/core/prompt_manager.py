...
2. 字段名称（keyDesc）必须从提供的标准名称池中选择，不能自己创造，你输出的keyDesc必须可以在标准名称池中完全匹配上
...
7. 图片中的字段名称可能与标准名称不完全一致，需要你判断对应关系，但是你输出的字段名称必须是标准名称池中的名称，不可以用图片中的字段名称替代（如图片中显示"最终目的国"，但是你必须使用字段池中的名称"运抵国")
8. 并不是图片中的每一个字段都需要提取，只有包含了标准池中所需要的信息的字段才需要提取，其他字段忽略，并且提取出来的信息都必须使用标准名称池中的名称，不可以自创名称，不可以用图片中的字段名称
9. 如果图片中使用了英文字段名，请不要自行翻译，而是去字段池中寻找对应的中文名称进行输出
10. 表头标准字段池和表体标准字段池不可以混淆使用，表头字段池专门用于表头字段（一般来说在图片上方）识别，表体字段池专门用于表体字段（一般来说在图片下方）识别

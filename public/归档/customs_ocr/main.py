...
async def main_async(input_json_path: str, output_json_path: str):
    ...
    try:
        ...
        print(f"未处理结果：{mainfactor_results}")
        # 处理申报要素识别结果
        mainfactor_results = process_mainfactors(mainfactor_results)

        print(f"申报要素识别结果：{mainfactor_results}")
        logger.info("申报要素识别完成")

        logger.info("聚合申报要素识别结果")
        aggregated_data = aggregate_mainfactors(aggregated_data, mainfactor_results)
        logger.info("聚合申报要素识别结果完成")
        print(f"[DEBUG] aggregated_data: {aggregated_data}")
        # 5. 异步检查一致性并统一
...
if __name__ == "__main__":
    # 默认路径
    input_path = "C:/desktop/YiBao/public/JSON/VLM识别报文/NOSNB25CL48865.json"
    output_path = "C:/desktop/YiBao/public/JSON/VLM识别报文/NOSNB25CL48865OUT.json"

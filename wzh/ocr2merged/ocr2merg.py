...
def extract_info_from_ocr(json_path: str) -> List[Dict[str, str]]:
    """
    从 ocr.json 文件中提取 operateImage 里的 imageId 和 attTypeCode

    :param json_path: ocr.json 文件路径
    :return: [
        {
            "imageId": "...",
            "attTypeCode": "..."
    ...
    for img in operate_images:
        attType = int(img.get("attTypeCode"))
        if attType == 2:
            imageId = int(img.get("imageId"))
...
def use_api(attType,imageUrl,imageId):
    match attType:
        case 0:
            prompt = "0"
        case 1:
...
```json
{
  ...
  "preDecList": [
    ...
    imageName = unquote(encoded_filename)

    with open(f"./api_file/{attType}_{imageName}.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

if __name__ == "__main__":
    images_info = extract_info_from_ocr("./ocr_file/OCR识别报文.json")
    for img in images_info:
        use_api(img["attType"],img["imageUrl"],img["imageId"])
